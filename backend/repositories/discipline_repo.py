...
class DisciplineRepository(BaseRepository):
    ...
    def get_teacher_discipline_ids(self, teacher_id: str):
        query = select(DisciplineTeacher.discipline_id).where(DisciplineTeacher.teacher_id == teacher_id)
        return [row[0] for row in self.db.execute(query).all()]

    def get_student_discipline_ids(self, student_id: str):
        query = select(DisciplineStudent.discipline_id).where(DisciplineStudent.student_id == student_id)
        return [row[0] for row in self.db.execute(query).all()]
